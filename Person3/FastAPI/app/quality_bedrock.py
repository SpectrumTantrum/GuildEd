...
# Model ID for Bedrock (Claude Haiku is fast and cheap)
...
def score_resources(topic: str, resources: List[dict], model_id: str | None = None) -> List[dict]:
    ...
    # Build prompt: one line per resource so we can parse back
    lines = []
    ...
    try:
        ...
        for block in content:
            if block.get("type") == "text":
                text = block.get("text", "")
                break
        # Parse lines like "0,0.9,0.8,0.95"
        out = list(resources)
        for line in text.strip().splitlines():
            ...
            try:
                ...
                if 0 <= idx < len(out):
                    # Combined score: weighted average (relevance weighted highest)
